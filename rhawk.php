<?php
exec("espeak -a 300 'welcome to ghost world'");
exec("espeak -a 300 'OPENING GHOST WEB SCANNING TERMINAL'");

$telegram_link = "https://t.me/TH3EL1T3GH0STOFFICIAL";
if (PHP_OS_FAMILY === 'Linux') {
    exec("xdg-open $telegram_link");
} elseif (PHP_OS_FAMILY === 'Windows') {
    exec("start $telegram_link");
} elseif (PHP_OS_FAMILY === 'Darwin') { // macOS
    exec("open $telegram_link");
} else {
    echo "Your operating system is not supported for this operation.";
}
error_reporting(0);
...
function update()
    {
        echo "\n\e[91m\e[1m[+] GHOST TOOLSUPDATE UTILITY [+]\nUpdate in progress, please wait...\n\n$cln";
        system("git fetch origin && git reset --hard origin/master && git clean -f -d");
        echo $bold . $fgreen . "[i] Job finished successfully! Please Restart GHOST TOOLS \n" . $cln;
        exit;
    }
...
elseif ($ip == "fix")
  {
    echo "\n\e[91m\e[1m[+] GHOST TOOLS FiX MENU [+]\n\n$cln";
